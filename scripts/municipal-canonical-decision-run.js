...
async function runCanonicalCity(city, options = {}) {
  ...
  return buildCanonicalDecisionObject({ ...run, resourceEnvelope: options.resourceEnvelope || null });
}

async function runCanonicalAll(options = {}) {
  const result = await runAll(options);
  const cities = result.cities.map(run => buildCanonicalDecisionObject({ ...run, resourceEnvelope: options.resourceEnvelope || null }));
  return {
