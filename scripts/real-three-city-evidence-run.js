...
const { requiredDataManifest, buildAcquisitionResult, normalizeRecord, retrieve } = require('../js/data-acquisition');
const { discoverInterventions } = require('../js/intervention-discovery');
...
async function buildAcquisitionContext(run, city, problem='homelessness', options={}) {
  const manifest=requiredDataManifest({objective:run.objective||'verified-outcome-improvement',problem,geography:city});
  const source={url:run.sourceLineage.sourceUrlUsed||run.sourceLineage.sourceUrl,provider:run.sourceLineage.provider,jurisdiction:city,domain:'local-baseline',tier:'official_publication',datasetId:run.sourceLineage.dataset||null};
  const snapshot=await retrieve(source,{fetchImpl:options.fetchImpl,now:options.now||new Date()});
  const observation=run.observedContext;
  const record=normalizeRecord({source,retrieval:snapshot.retrieval,value:observation?.value,unit:observation?.unit,period:observation?.asOf||observation?.period||'source-reported-period',geography:city,aggregation:observation?.aggregation||'source-reported',extractionMethod:observation?.extraction?.method||'municipal-adapter',definition:observation?.definition||null,asOf:observation?.asOf||null});
  const evidenceIndex={}; const housing=run.interventionComparison?.find(item=>item.id==='housing');
  if(housing?.gate?.causalEvidence&&housing.status==='ADMISSIBLE') evidenceIndex['housing-first-supportive-housing']={causal:{status:'supported',evidenceId:housing.gate.causalEvidence.id},implementation:{status:'potential',reason:'implementation evidence acquisition remains required'},cost:{status:'potential',reason:'decision-specific marginal cost acquisition remains required'},equity:{status:'potential',reason:'local equity-effect acquisition remains required'}};
  const candidates=discoverInterventions({problem,evidenceIndex});
  const gaps=[]; for(const candidate of candidates) for(const missing of candidate.missingEvidence) gaps.push(`intervention:${candidate.id}:${missing}`);
  return buildAcquisitionResult({manifest,candidates:[source],records:[record],gaps,snapshots:[snapshot.retrieval],interventionUniverse:candidates});
}

async function runRealEvidenceCity(city,options={}){const raw=await runCity(city,options);const evidenced=attachProductionEvidence(raw,city);const acquisition=await buildAcquisitionContext(evidenced,city,options.problem||'homelessness',options);const enriched=enrichEvidence({...evidenced,acquisition},options);const optimized=applyMarginalEvidence(enriched,options);return buildCanonicalDecisionObject({...optimized,acquisition,resourceEnvelope:options.resourceEnvelope||null,audit:{...optimized.audit,productionEvidenceRegistry:PRODUCTION_HOUSING_EVIDENCE[city].id,syntheticEvidenceExcluded:true,realMunicipalSource:true,realCausalEvidence:true,acquisitionComplete:acquisition.coverage.complete,acquisitionHash:acquisition.acquisitionHash}});}

async function runRealEvidenceAll(options={}){const cities=[];for(const city of ['Ottawa','Toronto','Melbourne']){try{cities.push(await runRealEvidenceCity(city,options));}catch(error){cities.push({identityBrief:{decisionId:`VIDIK-${city.toLowerCase()}-blocked`,city,objective:'verified-outcome-improvement',schemaVersion:'vidik.canonical-decision-object.v1',immutableSnapshot:true},rationale:{recommendation:null,why:'Real-evidence city run failed closed.',whyNot:[{id:city,failures:[error.message]}]},integrity:{decisionIntegrity:true,reproducibleRun:false,syntheticEvidenceExcluded:true},driftFailureRegistry:{failureClosed:true,failureReasons:[error.message]}});}}
return{schemaVersion:'vidik.real-three-city-evidence-decision.v1',decisionProblem:'Evaluate the same municipal homelessness resource-allocation problem using live municipal observations and explicitly provenance-bound causal evidence.',cities,evidenceRegistry:Object.fromEntries(Object.entries(PRODUCTION_HOUSING_EVIDENCE).map(([city,evidence])=>[city,{evidenceId:evidence.id,source:evidence.source,sourceUrl:evidence.sourceUrl,sourceJurisdiction:evidence.sourceJurisdiction,targetJurisdiction:evidence.targetJurisdiction,estimate:evidence.estimate,uncertainty:evidence.uncertainty,uncertaintyMethod:evidence.uncertaintyMethod||'source-reported',mode:evidence.mode}])),acceptance:{allCitiesReturned:cities.length===3,realMunicipalSourceRequired:true,realCausalEvidenceRequired:true,syntheticEvidenceExcluded:true,marginalOptimizationRequiresDecisionSpecificEvidence:true,acquisitionManifestRequired:true,interventionUniverseDiscoveryRequired:true}};}
