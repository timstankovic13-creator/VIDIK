...
function parseObservation(city, text) {
  ...
  return { city, dataset: spec.dataset, field: spec.field, unit: spec.unit, aggregation: spec.aggregation, role: spec.role, value: spec.value, definition: spec.definition };
}
...
function buildLearning(outcome = DEFAULT_OUTCOME_EXERCISE) {
  ...
  return { outcome: { ...outcome, error }, recalibration: { targetParameterId: 'housing:effect', adjustment, application: 'EXPLICIT_PARAMETER_MAPPING' }, drift: { detected: drift, threshold: 0.05, metric: 'absolute_prediction_error' } };
}
...
async function runCity(city, options = {}) {
  ...
  const comparison = INTERVENTIONS.map(intervention => { const gate = admissibility(city, intervention); return { ...intervention, gate, score: null, status: gate.admissible ? 'ADMISSIBLE' : 'BLOCKED' }; });
  ...
  const counterfactual = recommendation === 'housing' ? { intervention: 'housing', statusQuoEffect: 0, interventionEffect: CAUSAL.housing.estimate, incrementalEffect: CAUSAL.housing.estimate, evidenceIds: [CAUSAL.housing.id], semantics: 'Causal effect estimate is distinct from the municipal observed context.' } : null;
  const lineage = [{ evidenceId: `municipal:${city}:housing.need`, kind: 'observed_context', parameterId: 'housing.need', source: source.sourceUrl }, ...(recommendation ? [{ evidenceId: CAUSAL.housing.id, kind: 'causal_effect', parameterId: 'housing:effect', transportability: comparison.find(x => x.id === 'housing').gate.causalEvidence }] : [])];
  const learning = recommendation ? buildLearning(options.outcome) : null;
  return { schemaVersion: 'production-decision-run.v1', city, objective: 'verified-outcome-improvement', decisionState, decisionId: `VIDIK-${city.toLowerCase()}-${sha256({ city, observation, lineage }).slice(0, 12)}`, observedContext: observation, sourceLineage: source, interventionComparison: comparison, recommendation, recommendationName: comparison.find(x => x.id === recommendation)?.name || null, lineage, counterfactual, learning, audit: { generatedAt: retrievedAt, failureClosed: !recommendation, blockedAlternatives: comparison.filter(x => x.status === 'BLOCKED').map(x => ({ id: x.id, failures: x.gate.failures })), causalTransportability: comparison.find(x => x.id === 'housing').gate.causalEvidence, evidenceHash: sha256({ source, observation, lineage, counterfactual }) } };
}

async function runAll(options = {}) {
  const results = [];
  for (const city of ['Ottawa', 'Toronto', 'Melbourne']) results.push(await runCity(city, options));
  return { schemaVersion: 'production-three-city-acceptance.v1', decisionProblem: 'Allocate a fixed municipal resource pool among the same intervention universe subject to evidence/admissibility gates.', cities: results, comparison: results.map(r => ({ city: r.city, state: r.decisionState, recommendation: r.recommendation, observedField: r.observedContext.field, observedValue: r.observedContext.value })), acceptance: { Ottawa: results.find(r => r.city === 'Ottawa').decisionState === 'RECOMMENDATION' && results.find(r => r.city === 'Ottawa').learning, Toronto: results.find(r => r.city === 'Toronto').decisionState === 'RECOMMENDATION' && results.find(r => r.city === 'Toronto').learning, Melbourne: results.find(r => r.city === 'Melbourne').decisionState === 'BLOCKED' && results.find(r => r.city === 'Melbourne').audit.failureClosed } };
}
