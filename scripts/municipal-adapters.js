...
const ADAPTERS = Object.freeze({
  ...
  Melbourne: Object.freeze({
    sourceType: 'socrata',
    catalogUrl: 'https://data.melbourne.vic.gov.au/api/explore/v2.1/catalog/datasets/transport-activity-counts',
    mode: 'controlled-server-side',
    identityAuthority: 'GeoNames',
    populationEnrichment: 'WorldPop',
  }),
...
function validateCatalog(city, body) {
  ...
  if (city === 'Melbourne' && typeof body.dataset !== 'object') throw new Error('melbourne-dataset-missing');
...
async function ingestCatalog(city, fetchImpl = globalThis.fetch, now = new Date()) {
  const result = await inspectCatalog(city, fetchImpl);
  const records = city === 'Toronto' ? (result.body.result?.results || [])
    : city === 'Melbourne' ? [result.body.dataset]
    : (result.body.collections || result.body.links || [result.body]);
  return {
    city,
    ...
    provenance: provenance({ city, sourceUrl: result.sourceUrl, retrievedAt: now.toISOString(), records }),
  };
}
