...
const r=O.optimize({budget:100,interventions:[{id:'a',cost:50,maxCost:50,expectedValue:10},{id:'b',cost:50,maxCost:50,expectedValue:20}]});assert.equal(r.ok,true);assert.equal(r.exact,true);assert.equal(r.unallocated,0);assert.equal(r.fullEnvelopeCompared,true);assert.equal(r.allocation.length,2);
...
const granularity=O.optimize({budget:100,requireEvidence:true,interventions:[{id:'variable',cost:0,maxCost:100,expectedValue:10,evidenceStatus:'VERIFIED'}]});assert.equal(granularity.code,'ALLOCATION_GRANULARITY_REQUIRED');
...
console.log('VIDIK municipal budget optimizer 10.2: PASS — exact bounded portfolio, dependency/resource constraints, evidence/granularity gates');
