...
const { FAMILIES, buildUniverseAudit, planEvidence, causalGraph, transferability, allocate } = require('../src/full-scope/intelligence-hardening');

// 1. Unclassified candidates must remain unclassified; no silent family invention.
{
...
// 3. Malformed/effect-bearing discovery records cannot create recommendation authority.
{
...
// 4. One candidate cannot masquerade as a complete intervention universe.
{
...
// 5. Supported causal evidence is insufficient; causal evidence must be independently verified.
{
...
// 6. No source registry remains an acquisition gap, not zero evidence.
{
...
// 7. Two sources in one independence group do not count as independent causal evidence.
{
...
// 8. An unverified second source cannot unlock causality.
{
  const g = causalGraph({id:'x'}, [
...
// 11. Cross-legal-system mismatch is a hard transfer blocker.
{
...
// 13. Incomparable effect units block allocation before optimization.
{
  const a = allocate([
...
// 14. Fixed costs must consume budget; they cannot disappear from the allocation.
{
  ...
  assert.equal(a.selected[0].fixedCost, 60);
  assert.equal(a.selected[0].resource, 40);
  assert.equal(a.remainingBudget, 0);
}
...
// 16. Every intelligence-foundation output remains explicitly non-authoritative.
{
