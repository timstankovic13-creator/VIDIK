...
test('partial evidence outage is explicit and failure-closed', () => {
  ...
  assert.equal(result.stoppingReason, 'partial-evidence-source-failure');
  assert.equal(result.recommendationEligible, false);
});

test('duplicate evidence records collapse without importing effects', () => {
  ...
  assert.equal(Object.hasOwn(leads[0], 'causalEffect'), true);
});

test('caller cannot spoof an evidence source into authority', () => {
  assert.equal(sourceIsAuthoritative({ sourceId: 'openalex-works', jurisdiction: 'CA' }), true);
