...
test('partial source outage preserves usable leads but remains recommendation-blocked', async () => {
  ...
  console.error('PARTIAL_DEBUG', JSON.stringify(result)); assert.equal(result.candidates.length, 1); assert.equal(result.sourceSearches.filter(s => s.status === 'search-failed').length, 1); assert.equal(result.recommendationEligible, false);
});
