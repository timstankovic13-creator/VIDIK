...
test('requested jurisdiction cannot be bypassed by injecting another jurisdiction source', async () => {
  ...
  assert.deepEqual(result.sourcesSelected, []); assert.deepEqual(result.sourceSearches, []); assert.equal(result.candidates.length, 0); assert.equal(result.recommendationEligible, false);
...
test('international source remains eligible when a jurisdiction is requested', () => { const international = { ...CA, sourceId: 'ca-program-discovery', jurisdiction: 'international' }; const audit = buildApplicabilityAudit({ problem: 'housing', jurisdiction: 'CA', suppliedSources: [international] }); assert.equal(audit.rejectedSuppliedSources.length, 0); });
test('partial source outage preserves usable leads but remains recommendation-blocked', async () => {
  const result = await discoverSourceDrivenInterventions({ problem: 'food insecurity', jurisdiction: 'CA', sources: [CA, CA2], fetchImpl: async url => {
    if (url.includes('open.canada.ca')) return mockResponse({ result: { results: [{ id: 'p1', title: 'Community food access program', notes: 'Food support service.' }] } });
