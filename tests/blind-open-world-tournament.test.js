...
const {
  ...
  assertBlindRun,
} = require('../js/vidik-production-closed-loop');
    ...
    async search({ problem }) {
      ...
      return Array.from({ length: 3 }, (_, i) => ({
        ...
        provenance: [
          {
            source: `blind-${seed}`,
            queryHash: crypto.createHash('sha256').update(problem).digest('hex'),
          },
        ],
      }));
    },
...
const evidenceSource = {
  id: 'blind-evidence',
  async search({ candidate }) {
    return [
      {
        id: `lead-${candidate.id}`,
        candidateId: candidate.id,
        provenance: { externalId: `external-${candidate.id}` },
...
async function main() {
  const problems = [
    'nighttime pedestrian injury exposure',
    'heat vulnerability in dense blocks',
    'food access interruptions',
    'wildfire smoke exposure',
    'worker displacement after closure',
    'stormwater flooding at transit stops',
    'avoidable emergency demand',
    'rental instability after income shock',
    'school travel injury risk',
    'winter shelter capacity pressure',
    'urban tree mortality',
    'industrial noise exposure',
    'late-night collision risk',
    'opioid response demand',
    'household energy burden',
    'local air quality alerts',
    'construction worker heat exposure',
    'isolation among older residents',
    'vacant property hazards',
    'bus reliability gaps',
    'neighbourhood cooling deficits',
    'eviction filing pressure',
    'fire response travel delay',
    'safe cycling network gaps',
  ];
  ...
  for (let i = 0; i < problems.length; i++) {
    ...
    const verifications = Object.fromEntries(
      discovered.map((candidate, index) => {
        const estimate = 1 + ((hashNumber(candidate.id) + index) % 9);
        return [
          `lead-${candidate.id}`,
          {
            ...
            parameter: {
              ...
              unit: 'outcome/CAD',
              uncertainty: {
                low: Math.max(0.5, estimate - 0.25),
                high: estimate + 0.25,
              },
            },
          },
    ...
    const candidateIds = discovered.map((candidate) => candidate.id);
    const result = await executeProductionDecision({
      ...
      targetJurisdiction: 'BLIND',
      resourceEnvelope: {
        marginalUnit: { amount: 100, unit: 'CAD' },
      },
    ...
    assert.ok(
      result.lineage.discoveryHash &&
        result.lineage.evidenceHash &&
        result.lineage.parameterHash &&
        result.lineage.decisionInputsHash &&
        result.lineage.optimizerHash &&
        result.lineage.decisionIntelligenceHash &&
        result.lineage.artifactHash,
    );
    results.push(result);
  }
  ...
  assert.equal(
    new Set(results.map((result) => result.discovery.discoveryHash)).size,
    problems.length,
  );
  console.log(
    `BLIND OPEN-WORLD TOURNAMENT: ${results.length}/${problems.length} passed; no gold intervention answers supplied.`,
  );
}
