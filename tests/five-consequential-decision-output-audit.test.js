  ...
  for (const caseDef of CASES) {
    ...
    audit.push({
      ...
      candidates: run.candidates.map(candidate => ({
        id: candidate.id,
        name: candidate.name,
        sourceType: candidate.discovery?.sourceType || null,
        sourceId: candidate.discovery?.sourceId || null,
        leadOnly: candidate.discovery?.leadOnly,
        effectsImported: candidate.discovery?.effectsImported,
        evidenceGate: run.governance.evidenceToDecisionGates?.[candidate.id]?.gates || null,
        evidenceGateReasons: run.governance.evidenceToDecisionGates?.[candidate.id]?.reasons || null
      })),
      lifecycle: run.governance.decisionLifecycle,
      candidateUniverse: run.governance.candidateUniverseIntelligence,
      whyWhyNot: run.nextPhase?.whyWhyNot,
      sourceNetwork: run.nextPhase?.sourceNetwork,
      evidenceSearches: run.evidenceSearches,
      evidenceLeadCount: run.evidenceDiscovery.reduce((n, item) => n + (item.evidenceLeads?.length || 0), 0)
    });
