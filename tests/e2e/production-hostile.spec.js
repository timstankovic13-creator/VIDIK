...
async function openAcceptance(page){await page.locator('#seeAnalysis').click();await page.locator('details.advanced').filter({hasText:'Security, acceptance & adapters'}).locator('summary').click();}
...
test('production hostile: canonical app exposes acceptance controls',async({page})=>{await page.goto('/');await openAcceptance(page);await expect(page.locator('#runAcceptance')).toBeVisible();await expect(page.locator('#acceptanceLog')).toBeVisible()});
