...
test('Case 001 Ottawa blind runtime — strict temporal boundary, normalized provenance, and marginalization gates', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    Object.assign(E, {
      'S01-draft-budget-2024': { id: 'S01-draft-budget-2024', title: 'Ottawa Draft Budget 2024', sourceType: 'municipal-budget', url: 'https://documents.ottawa.ca/sites/default/files/Document%205%20-%20Draft%20Budget%202024%20Report.pdf', publishedAt: '2023-11-08', publicationDateVerified: true, retrievedAt: '2026-09-01', status: 'verified', quality: 'high', transportability: 1, claims: [{ id: 'S01-C01', type: 'planning-context', text: 'Draft Budget 2024 was tabled November 8, 2023 before the December 6 adoption boundary.' }] },
      'S02-census-2021': { id: 'S02-census-2021', title: 'Statistics Canada 2021 Census Profile — Ottawa', sourceType: 'census', url: 'https://www12.statcan.gc.ca/census-recensement/2021/dp-pd/prof/', publishedAt: '2023-11-15', publicationDateVerified: true, retrievedAt: '2026-09-01', status: 'verified', quality: 'high', transportability: .95, claims: [{ id: 'S02-C01', type: 'structural-context', text: 'Ottawa had 146,985 tenant households; 35.1% spent 30% or more of income on shelter; 23.3% were in core housing need.' }] },
      'S03-housing-rct': { id: 'S03-housing-rct', title: 'At Home/Chez Soi Housing First randomized evidence', sourceType: 'research-trial', url: 'https://mentalhealthcommission.ca/resource/national-at-home-chez-soi-final-report/', publishedAt: '2016-04-14', publicationDateVerified: true, retrievedAt: '2026-09-01', status: 'verified', quality: 'high', transportability: .78, claims: [{ id: 'S03-C01', type: 'causal', text: 'A five-city Canadian RCT of 950 high-need participants found 73% stable housing in Housing First versus 31% in treatment as usual at one year; adjusted difference 42 percentage points (95% CI 36–48).' }] },
      'S04-ottawa-housing-2022-update': { id: 'S04-ottawa-housing-2022-update', title: 'Ottawa 2022 Housing and Homelessness Update', sourceType: 'municipal-program-report', url: 'https://documents.ottawa.ca/en/files/2022-housing-and-homelessness-update', publishedAt: null, publicationDateVerified: false, retrievedAt: '2026-09-01', status: 'verified', quality: 'high', transportability: 1, claims: [{ id: 'S04-C01', type: 'program-output', text: '151 individuals were housed through Housing First services from January to September 2022.' }, { id: 'S04-C02', type: 'retention', text: '82% of singles in Housing First retained housing one year after becoming housed.' }, { id: 'S04-C03', type: 'capacity', text: '12 new Housing Based Case Managers were added to support over 150 more clients.' }, { id: 'S04-C04', type: 'prevention', text: 'Family homelessness prevention case conferencing reported 93% success (28 of 30 cases did not enter shelter).' }], notes: 'Exact publication date is not verified; strict temporal admissibility excludes this source until verified.' }
    });
    ...
    if (housing) { housing.params.need = null; housing.params.capacity = null; housing.params.feasibility = null; housing.params.effect.evidenceIds = ['S03-housing-rct']; }
    for (const c of C.filter(x => x.id !== 'housing')) { c.params.need = null; c.params.effect = null; c.params.capacity = null; c.params.feasibility = null; }
    const reconstruction = VIDIK_HistoricalParameters.reconstruct({
      boundary, sources: E, candidateId: 'housing',
      claimRules: [
        { parameterType: 'need', sourceIds: ['S02-census-2021'], claimType: 'structural-context', unit: 'proportion of tenant households', value: .351, denominator: '146,985 tenant households', geography: 'Ottawa', population: 'tenant households', measurementPeriod: '2021 Census', uncertainty: 'descriptive Census estimate; not a candidate-specific marginal need', candidateSpecific: false, requiresCandidateSpecific: true, method: 'contextual housing-cost burden; cannot be substituted for Housing First eligibility or marginal need', reason: 'candidate-specific need remains unsupported' },
      ...
      marginalMap: { resourceUnit: 'CAD', resourceAmount: 1000000, capacityUnit: 'additional Housing First placements', capacityValue: null, outcomeUnit: 'additional stable-housing participant-years', outcomeValue: null, sourceIds: ['S03-housing-rct'], causalGate: 'not-passed', transportabilityGate: 'not-passed' }
    });
    document.getElementById('city').value = 'Ottawa'; document.getElementById('pool').value = '1000000'; document.getElementById('risk').value = '0.75'; render();
    return { case: 'OTTAWA_CASE001', mode: 'blind', boundary, admissibleSourceIds: admissible, excludedSourceIds: Object.keys(E).filter(id => !admissibility[id].admissible), sealedSourceIds: ['X01-adopted-budget-2024','X02-2023-progress-report','X03-cmhc-2024-rental-report'], coreEngineVersion: V.version, lifecycleVersion: '9.6.1', reconstruction, recommendation: document.getElementById('rec').textContent || null, gate: document.getElementById('gate').textContent || null, candidateText: document.getElementById('candidates').innerText || '', audit: document.getElementById('audit').textContent || '' };
  });
  ...
  expect(result.reconstruction.marginalization.status).toBe('missing'); expect(result.reconstruction.blocked).toBe(true);
  expect(result.recommendation).toBe('NO RECOMMENDATION'); expect(result.gate).toContain('BLOCKED'); expect(result.candidateText).toContain('BLOCKED');
...
test('Case 001 reconstruction adversarial gates reject temporal leakage and assumption laundering', async ({ page }) => {
  await page.goto('/'); await page.waitForFunction(() => document.readyState === 'complete');
  const result = await page.evaluate(() => {
    const boundary = '2023-12-06';
  ...
  expect(result.temporal.admissible).toBe(false); expect(result.temporal.reason).toBe('published after historical boundary');
  expect(result.unknownDate.admissible).toBe(false); expect(result.unknownDate.reason).toBe('publication date not verified');
  expect(result.assumptionStatus).toBe('missing'); expect(result.assumptionReason).toContain('assumption laundering gate');
});
