...
test.describe('VIDIK 9.2 decision intelligence acceptance',()=>{
  ...
  test('3 evidence conflict is surfaced',async({page})=>{await page.goto('/');await di(page);expect(await page.evaluate(()=>{const r=window.VIDIK_DECISION_INTELLIGENCE_92.resolveEvidenceConflict([{id:'a',claimKey:'q',direction:'up',quality:.8},{id:'b',claimKey:'q',direction:'down',quality:.7}]);return r[0].resolution;})).toBe('CONFLICT_REQUIRES_REVIEW');});
  ...
  test('5 correlated uncertainty includes covariance',async({page})=>{await page.goto('/');await di(page);expect(await page.evaluate(()=>{const r=window.VIDIK_DECISION_INTELLIGENCE_92.correlatedUncertainty([{id:'a',mean:1,low:.8,high:1.2},{id:'b',mean:1,low:.8,high:1.2}],[{a:'a',b:'b',rho:.5}]);return r.variance;})).toBeGreaterThan(0);});
  test('6 sensitivity detects recommendation flips',async({page})=>{await page.goto('/');await di(page);expect(await page.evaluate(()=>window.VIDIK_DECISION_INTELLIGENCE_92.sensitivityFlip({baseline:{x:.25},parameters:[{id:'x',low:0,high:1}],scoreFn:s=>({recommendation:s.x>.5?'B':'A'})}))).toMatchObject({recommendationStable:false});});
