...
for(const item of discovered){
  ...
  if(state==='ADMISSIBLE')Object.assign(rec,{evidenceIds:['fixture'],cost:0,maxCost:item.id==='ps-cvi'?100000:0,increment:item.id==='ps-cvi'?100000:undefined,capacity:1,expectedValue:item.id==='ps-cvi'?10:0,uncertainty:{low:0,high:1},constraints:{legal:[],implementation:[],capacity:[]}});
...
r=P.evaluate(d);assert.strictEqual(r.ok,true,'complete disposition + admissible evidence should pass the universe gate');
...
const portfolio=P.optimizeMunicipalPortfolio(d,'full-request');assert.ok(portfolio.ok===true||portfolio.code==='NO_FEASIBLE_PORTFOLIO','portfolio path must optimize or fail closed, never silently rank');if(portfolio.ok)assert.strictEqual(portfolio.solver.fullEnvelopeCompared,true);
...
const bad=P.setMunicipalUniverseDisposition({problem,universeId:'does-not-exist',state:'ADMISSIBLE',rationale:'bad'});assert.strictEqual(bad.ok,false);assert.strictEqual(bad.code,'UNKNOWN_UNIVERSE_ITEM');
