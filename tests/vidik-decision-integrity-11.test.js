...
let d=P.createDecision({audience:'municipal',objective:'violent crime reduction',problem,jurisdiction:'Ottawa',statusQuo:{description:'Existing municipal allocation'},options:[{id:'placeholder'}],evidence:[{id:'fixture'}],uncertainty:{overall:0.5},opportunityCost:{known:true},equity:{assessed:true},constraints:{budget:100000},provenance:[{id:'fixture'}]});
...
for(const item of discovered){
  ...
  const rec={problem,jurisdiction:'Ottawa',universeId:item.id,state,rationale:state==='ADMISSIBLE'?'Decision baseline or evidence-backed fixture for integrity test.':'Explicitly classified outside the admissible set for this decision test; separate evidence review remains required.',objectiveMetric:'violent crime reduction'};
  if(state==='ADMISSIBLE'){
...
d=P.createDecision({audience:'municipal',objective:'violent crime reduction',problem,jurisdiction:'Ottawa',statusQuo:{description:'Existing municipal allocation'},options:[{id:'placeholder'}],evidence:[{id:'fixture'}],uncertainty:{overall:0.5},opportunityCost:{known:true},equity:{assessed:true},constraints:{budget:100000},provenance:[{id:'fixture'}]});
r=P.evaluate(d);assert.strictEqual(r.ok,true,'complete disposition + admissible verified evidence should pass the universe gate');
...
const bad=P.setMunicipalUniverseDisposition({problem,jurisdiction:'Ottawa',universeId:'does-not-exist',state:'ADMISSIBLE',rationale:'bad'});assert.strictEqual(bad.ok,false);assert.strictEqual(bad.code,'UNKNOWN_UNIVERSE_ITEM');
