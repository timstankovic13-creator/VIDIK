...
function buildLineage({evidence=[],claims=[],parameters=[],recommendation=null}){const E=new Map(evidence.map(x=>[x.id,x]));const C=new Map(claims.map(x=>[x.id,x]));const P=new Map(parameters.map(x=>[x.id,x]));const links=[];for(const p of parameters){for(const cid of (p.claimIds||[])){const c=C.get(cid);if(!c)throw new Error('missing-claim:'+cid);for(const eid of (c.evidenceIds||[])){if(!E.has(eid))throw new Error('missing-evidence:'+eid);links.push({evidenceId:eid,claimId:cid,parameterId:p.id});}}}if(recommendation){for(const pid of (recommendation.parameterIds||[]))if(!P.has(pid))throw new Error('missing-parameter:'+pid);}return {version:DI92_VERSION,links,hash:hashObject(links)};}
function propagateEvidenceChange({before,after,recommendationFn}){const beforeHash=hashObject(before),afterHash=hashObject(after);const beforeResult=recommendationFn(before);const afterResult=recommendationFn(after);return {evidenceChanged:beforeHash!==afterHash,beforeHash,afterHash,recommendationChanged:hashObject(beforeResult)!==hashObject(afterResult),before:beforeResult,after:afterResult};}
