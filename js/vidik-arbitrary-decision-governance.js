...
const EVIDENCE_STATUSES = new Set(['verified', 'supported', 'complete']);
const BLOCKING_STATUSES = new Set(['blocked', 'stale', 'conflicting', 'missing', 'unverified']);
...
function buildSemanticExpansion({ problem, synonyms = {}, mechanisms = {}, interventionClasses = {} } = {}) { const base = String(problem || '').trim().toLowerCase(); if (!base) throw new Error('semantic-expansion-problem-required'); const tokens = base.split(/\s+/).filter(Boolean); const expanded = new Set([base, ...tokens]); const add = value => { if (Array.isArray(value)) value.forEach(v => expanded.add(String(v).trim().toLowerCase())); else if (value) expanded.add(String(value).trim().toLowerCase()); }; for (const token of tokens) add(synonyms[token]); add(synonyms[base]); add(mechanisms[base]); add(interventionClasses[base]); return { base, queries: [...expanded].filter(Boolean).slice(0, 20), queryHash: sha256([...expanded].sort()) }; }
function buildParameterBridge({ candidate, evidence = {}, resource = {}, effect = {} } = {}) { const causal = evidence.causal || {}; const value = Number(effect.value ?? causal.effectEstimate); const resourceValue = Number(resource.value ?? candidate?.resourceRequirement); const unit = effect.unit || causal.effectUnit || null; const resourceUnit = resource.unit || candidate?.resourceUnit || null; const admissible = Number.isFinite(value) && Number.isFinite(resourceValue) && resourceValue > 0 && Boolean(unit) && Boolean(resourceUnit) && Boolean(causal.independentSource || causal.verification?.independent === true || causal.verifiedBy); return { admissible, parameter: admissible ? { value, unit, sourceId: causal.sourceId || null } : null, marginal: admissible ? { effect: value, effectUnit: unit, resource: resourceValue, resourceUnit } : null, blockedReason: admissible ? null : 'parameter-resource-effect-insufficient' }; }
function buildOutcomeClosure({ decisionArtifactHash, observations = [], reviewIntervals = [6, 12, 24, 60] } = {}) { const valid = observations.filter(o => o && Number.isFinite(Number(o.observed)) && o.metric); const next = reviewIntervals.filter(months => !observations.some(o => Number(o.months) === months)); return { baselineHash: decisionArtifactHash || null, observedCount: valid.length, reviewsDueMonths: next, attributionRequired: true, driftCheckRequired: true, learningEligible: valid.length > 0, historyImmutable: true, proposedParameterChanges: [] }; }
function monsterCase({ problem, candidate, evidence, statusQuo, analysisInputs = {}, learning = {}, comparableCities = [], sourceSearches = null } = {}) { const verification = verifyEvidenceBundle({ candidate, evidence }); const provenanceSources = Array.isArray(candidate?.discovery?.provenance) ? candidate.discovery.provenance.filter(p => p?.sourceId).map(p => ({ sourceId: p.sourceId, status: 'candidates-found' })) : []; const searches = sourceSearches || (provenanceSources.length ? provenanceSources : [{ sourceId: 'monster-fixture', status: 'candidates-found', candidatesReturned: 1 }]); const universe = buildCandidateUniverseIntelligence({ candidates: candidate ? [candidate] : [], sourceSearches: searches, statusQuo }); const learningDiscovery = buildLearningDiscoveryLeads({ problem, learning, comparableCities }); const lifecycle = buildDecisionLifecycle({ problem, discovery: true, evidenceVerification: verification, universe, learningDiscovery, downstream: { decision: false } }); const recommendationAllowed = verification.status === 'verified' && universe.sufficientForRecommendation === true && statusQuo?.explicit === true && !candidate?.discovery?.leadOnly && Number.isFinite(Number(analysisInputs.estimate)); const blockedReasons = []; if (verification.status !== 'verified') blockedReasons.push('evidence-verification'); if (!universe.sufficientForRecommendation) blockedReasons.push('candidate-universe'); if (statusQuo?.explicit !== true) blockedReasons.push('status-quo'); if (candidate?.discovery?.leadOnly === true) blockedReasons.push('lead-only-candidate'); if (!Number.isFinite(Number(analysisInputs.estimate))) blockedReasons.push('non-finite-effect-estimate'); return { status: recommendationAllowed ? 'PASS' : 'BLOCKED', verification, universe, learningDiscovery, lifecycle, recommendationAllowed, blockedReasons }; }
module.exports = { REQUIRED_EVIDENCE, evidenceRecordStatus, verifyEvidenceBundle, buildCandidateUniverseIntelligence, normalizeComparableInterventions, buildLearningDiscoveryLeads, buildDecisionLifecycle, buildEvidenceToDecisionGate, buildSemanticExpansion, buildParameterBridge, buildOutcomeClosure, monsterCase, sha256 };
