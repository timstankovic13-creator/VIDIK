...
function evidenceRecordStatus(record) {
  ...
  if (record.freshness?.stale === true || record.freshness?.status === 'stale') return 'stale';
  if (record.conflict === true || record.status === 'conflicting') return 'conflicting';
...
function verifyEvidenceBundle({ candidate, evidence = {}, now = new Date(), requiredEvidence = null } = {}) {
  ...
  for (const type of required) {
    const record = evidence?.[type];
    const status = evidenceRecordStatus(record);
    const independent = Boolean(record?.independentSource || record?.verification?.independent === true || record?.verifiedBy);
    const causal = type === 'causal' ? Boolean(record?.causal === true || record?.causalIdentification || record?.effectEstimate !== undefined) : true;
    checks[type] = { status, independent, causal, sourceId: record?.sourceId || record?.provenance?.sourceId || null, retrievedAt: record?.retrievedAt || null };
  }
  const missing = Object.entries(checks).filter(([, c]) => c.status === 'missing').map(([type]) => type);
  const blocked = Object.entries(checks).filter(([, c]) => BLOCKING_STATUSES.has(c.status) || c.status === 'unverified').map(([type]) => type);
  const independentMissing = Object.entries(checks).filter(([, c]) => !c.independent).map(([type]) => type);
...
function buildCandidateUniverseIntelligence({ candidates = [], sourceSearches = [], statusQuo = null } = {}) {
  ...
  for (const candidate of candidates) {
    ...
    const provenance = candidate.discovery?.provenance || [];
