...
function allocateFiveMillion(scenario, riskCeiling = 1) {
  ...
  return {
    blocked: false, synthetic: true, pool: POOL, riskCeiling,
    allocations, unallocated: remaining,
    objectiveValue: eligible.reduce((sum,c) => sum + allocations[c.id] * c.valuePerDollar, 0),
    productionRecommendation: null, effectEstimate: null, roi: null,
    status: 'LAB_ONLY_NOT_A_REAL_RECOMMENDATION'
  };
}

// Perturb one candidate at a time. This makes recommendation-flip testing meaningful.
function sensitivity(scenario, riskCeiling = 1, deltas = [-0.25,-0.10,0,0.10,0.25]) {
  return deltas.map(delta => {
    const rows = scenario.candidates.map((c, i) => ({
      candidate: c.id,
      delta,
      result: allocateFiveMillion({ ...scenario, candidates: scenario.candidates.map((x,j) => j === i ? { ...x, valuePerDollar: Math.max(0, x.valuePerDollar * (1 + delta)) } : { ...x }) }, riskCeiling)
    }));
    return { delta, rows };
  });
}
