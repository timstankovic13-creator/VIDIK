'use strict';
// VIDIK 9.2.1 canonical integration: evidence-backed VOI and counterfactual semantics.
(function(){
  ...
  async function recompute(){
    const claims=claimsForEvidence(),parameters=C.flatMap(paramsForCandidate),scored=C.map(score),risk=numericControl('risk');
  ...
  state.recompute=recompute;const originalRender=window.render;window.render=function(){const r=originalRender();recompute().catch(e=>{state.status='BLOCKED';state.error=e.message;});return r;};window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>recompute().catch(e=>{state.status='BLOCKED';state.error=e.message;}),0));
})();
