...
function normalizeResourceEnvelope(resourceEnvelope) {
  ...
  const unit = String(resourceEnvelope?.marginalUnit?.unit || 'CAD').toUpperCase();
  if (unit !== 'CAD') return { status: 'INVALID', amount: null, unit, feedback: 'VIDIK production resource translation requires CAD-denominated municipal resources; currency conversion must be explicit before optimization.' };
  return { status: 'VALID', amount: finitePositive(amount, 'marginal-resource-amount-must-be-positive-finite'), unit };
}
...
function validateModel(intervention, model) {
  ...
  if (!Array.isArray(model.evidenceIds) || model.evidenceIds.length < 3) failures.push('resource-chain-evidence-lineage-incomplete');
  if (Array.isArray(model.evidenceIds) && new Set(model.evidenceIds).size !== model.evidenceIds.length) failures.push('duplicate-evidence-lineage');
...
function evaluateResourceOptimization(resourceEnvelope, interventionComparison, resourceModels = {}) {
  ...
  if (resource.status === 'NOT_SPECIFIED') return { status: 'NOT_ACTIVATED', resource, candidates: [], allocation: null, opportunityCost: null, feedback: resource.feedback };
  if (resource.status !== 'VALID') return { status: 'BLOCKED', resource, candidates: [], allocation: null, opportunityCost: null, feedback: resource.feedback };
  ...
  const winner = ranked[0], runnerUp = ranked[1] || null;
