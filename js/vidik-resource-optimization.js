...
function evaluateResourceOptimization(resourceEnvelope, interventionComparison, resourceModels = {}) {
  ...
  const candidates = interventionComparison.map(intervention => {
    ...
    const duplicateLineage = Boolean(model?.evidenceIds && Array.isArray(model.evidenceIds) && new Set(model.evidenceIds).size !== model.evidenceIds.length);
    if (duplicateLineage) { if (!validation.failures.includes('resource-chain-evidence-lineage-incomplete')) validation.failures.push('resource-chain-evidence-lineage-incomplete'); validation.failures.push('resource-chain-evidence-lineage-duplicate'); validation.admissible = false; }
  ...
  const duplicateBlocked = candidates.filter(x => x.status === 'BLOCKED' && x.failures.includes('resource-chain-evidence-lineage-duplicate'));
  if (duplicateBlocked.length) return { status: 'BLOCKED', resource, candidates, allocation: null, opportunityCost: null, feedback: `Resource optimization is blocked because no admissible candidate has a complete evidenced marginal resource-to-outcome chain; candidate evidence lineage contains duplicate evidence identifiers: ${duplicateBlocked.map(x => x.id).join(', ')}.` };
