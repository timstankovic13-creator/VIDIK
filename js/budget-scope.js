...
function detectBudgetIntent(text) {
  ...
  return {
    requested: Boolean(fullBudget || optimization || valuePreservation || amount),
    scope,
    amount: amount?.amount || null,
    currency: amount?.currency || null,
    objective: valuePreservation ? 'preserve-marginal-dollar-value' : (optimization ? 'optimize-resource-allocation' : null),
    fullBudget,
    optimizationRequested: optimization,
    valuePreservationRequested: valuePreservation
  };
}
