...
const DEFAULT_POLICY = Object.freeze({
  minQuality: 0.70,
  maxFreshnessDays: 365,
  minTransportabilitySimilarity: 0.80,
  requireSourceJurisdiction: true,
  requireTargetJurisdiction: true,
  rejectScenarioEvidence: true
});
...
function daysOld(asOf, now = new Date()) {
...
function normalizeCurrency(unit) {
  const text = String(unit || '').trim().toUpperCase();
  const match = text.match(/\b(CAD|USD|AUD|EUR|GBP)\b/);
...
function unitsCompatible(expected, actual) {
  ...
  const e = normalizeCurrency(expected), a = normalizeCurrency(actual);
  return Boolean(e && a && e === a);
}

function transportabilitySimilarity(target, source) {
  const dimensions = [
    'populationScale', 'problemDefinition', 'outcomeDefinition', 'serviceSystem',
    'institutionalContext', 'demographicContext', 'implementationContext', 'temporalCompatibility'
  ];
  const scores = dimensions.map(key => {
    ...
    if (finiteNumber(a) && finiteNumber(b)) {
      ...
      return Math.max(0, 1 - Math.abs(av - bv) / scale);
    }
    return a === b ? 1 : 0;
  }).filter(v => v !== null);
...
function enforceEvidenceAdmissibility(evidence, context = {}, policy = {}) {
  ...
  if (freshness && daysOld(freshness, context.now) > p.maxFreshnessDays) failures.push('evidence-stale');
  ...
  if (context.expectedJurisdiction && evidence.sourceJurisdiction && evidence.targetJurisdiction) {
    const source = String(evidence.sourceJurisdiction);
    const target = String(evidence.targetJurisdiction);
    const exact = source.toLowerCase() === String(context.expectedJurisdiction).toLowerCase() || target.toLowerCase() === String(context.expectedJurisdiction).toLowerCase();
    const sameCountry = source.slice(0, 2).toUpperCase() === String(context.expectedJurisdiction).slice(0, 2).toUpperCase();
  ...
  return {
    ...
    failures,
    transportabilitySimilarity: evidence.transportabilitySimilarity ?? transportabilitySimilarity(context.targetProfile, evidence.sourceProfile || evidence.transportabilityProfile),
    policy: p
  };
}
...
function validateEvidenceSet(evidenceItems, context = {}, policy = {}) {
  ...
  for (const item of items) {
    if (item?.id && seen.has(item.id)) duplicateIds.push(item.id);
