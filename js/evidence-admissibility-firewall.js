...
function normalizeCountryCode(value) {
  ...
  const aliases = { canada: 'CA', 'canada,': 'CA', australia: 'AU', 'united states': 'US', 'united states of america': 'US', usa: 'US', 'new zealand': 'NZ', 'united kingdom': 'GB', uk: 'GB' };
  if (aliases[text]) return aliases[text];
  const code = text.match(/(?:^|[\s,(/-])([a-z]{2})(?:$|[\s,)/-])/i);
  return code ? code[1].toUpperCase() : null;
}
function jurisdictionCountryCode(value) {
...
function enforceEvidenceAdmissibility(evidence, context = {}, policy = {}) {
  ...
  if (context.expectedJurisdiction && evidence.sourceJurisdiction && evidence.targetJurisdiction) {
    ...
    const sourceCountry = jurisdictionCountryCode(source), targetCountry = jurisdictionCountryCode(target), expectedCountry = jurisdictionCountryCode(expected);
    const sameCountry = Boolean(expectedCountry && (sourceCountry === expectedCountry || targetCountry === expectedCountry));
...
module.exports = { DEFAULT_POLICY, daysOld, unitsCompatible, normalizeCountryCode, jurisdictionCountryCode, transportabilitySimilarity, enforceEvidenceAdmissibility, validateEvidenceSet };
