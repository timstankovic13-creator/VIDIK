...
function buildGovernedDiscoveryPlan({ objective, problem, geography }) {
  // Do not require exact problem-token matches to find governed sources. Exact tag
  // filtering is a ranking signal, not an admissibility gate for arbitrary domains.
...
function buildInterventionUniverseSources({ problem, geography, providedSources = [] } = {}) {
  ...
  for (const source of sources) for (const query of queries) {
    ...
    expanded.push({
      ...
      extractionMethod: 'governed-catalog-discovery'
    });
  }
  return expanded;
}
