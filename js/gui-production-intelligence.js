...
(function(){
  ...
  function render(){
    ...
    const input=document.getElementById('decisionProblem'); const problem=(input?.value||'').trim();
    const candidates=Array.isArray(window.C)?window.C:[];
    const typed=problem.length>0;
    const hasCandidates=candidates.length>0;
    const engine=window.VIDIK_PRODUCTION_INTELLIGENCE;
    const readiness=engine?.buildReadiness({discoveryComplete:typed&&hasCandidates,evidenceVerified:false,parametersAdmissible:false,optimizationValid:false,decisionArtifactPersisted:false,outcomeLearningReady:false});
    root.innerHTML='<div class="gui-engine-kicker">LIVE ENGINE STATE</div><div class="gui-engine-grid">'+
      '<div><span>01 · Discovery</span>'+state(typed&&hasCandidates,'Problem + candidate universe')+'</div>'+
      ...
      '<div><span>06 · Learning</span>'+state(false,'Outcome review / recalibration')+'</div>'+
      '</div><div class="gui-engine-note">'+(readiness?esc(readiness.passed)+' of '+esc(readiness.total)+' production layers currently satisfied by this browser surface. Discovery never counts as evidence or recommendation eligibility.':'Engine readiness bridge unavailable — no substitute result is shown.')+'</div>';
  }
  function inject(){
  ...
  function init(){
    ...
    if(anchor&&!document.getElementById('guiEngineReadiness')){const section=document.createElement('section');section.id='guiEngineReadiness';section.className='card gui-engine';anchor.parentNode.insertBefore(section,anchor);}
    render();
    document.getElementById('decisionProblem')?.addEventListener('input',render);
  }
  window.VIDIK_GUI_PRODUCTION={render};
