...
function analyzeStream(caseId, input = {}) {
  ...
  const preregistration = input.preregistrationFrozen === true;
  ...
  const effectReady = decisionSupportReady && preregistration && allocation && causal && measurement;
  ...
  const missingFor = required => required.filter(field => !fields.has(field));
