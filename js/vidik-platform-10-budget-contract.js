...
(function (w) {
  const TYPES = Object.freeze([
    'STATUS_QUO', 'REQUESTED', 'AVAILABLE', 'OPERATING', 'CAPITAL',
    'PROGRAM', 'SERVICE', 'PREVENTION', 'RESPONSE', 'INFRASTRUCTURE',
    'HOUSING', 'HEALTH', 'PUBLIC_SAFETY', 'ENVIRONMENT', 'TRANSPORT',
    'EQUITY', 'RESERVE', 'CONTINGENCY', 'FULL_MUNICIPAL'
  ]);
  const TYPE_KEYS = Object.freeze({
    STATUS_QUO:'status-quo', REQUESTED:'requested', AVAILABLE:'available', OPERATING:'operating', CAPITAL:'capital',
    PROGRAM:'program', SERVICE:'service', PREVENTION:'prevention', RESPONSE:'response', INFRASTRUCTURE:'infrastructure',
    HOUSING:'housing', HEALTH:'health', PUBLIC_SAFETY:'public-safety', ENVIRONMENT:'environment', TRANSPORT:'transport',
    EQUITY:'equity', RESERVE:'reserve', CONTINGENCY:'contingency', FULL_MUNICIPAL:'full-municipal'
  });
  ...
  function generateMunicipalBudgetOptions(input) {
    ...
    const values = { 'status-quo':statusQuoBudget, requested:requestedBudget, available:availableBudget, operating:requestedBudget, capital:requestedBudget, program:requestedBudget, service:requestedBudget, prevention:requestedBudget, response:requestedBudget, infrastructure:requestedBudget, housing:requestedBudget, health:requestedBudget, 'public-safety':requestedBudget, environment:requestedBudget, transport:requestedBudget, equity:requestedBudget, reserve:Math.max(0, availableBudget-requestedBudget), contingency:Math.max(0, availableBudget-requestedBudget), 'full-municipal':requestedBudget };
  ...
  function attach(P){
    if(!P)return false; P.MUNICIPAL_BUDGET_TYPES=P.MUNICIPAL_BUDGET_TYPES||TYPES; P.generateMunicipalBudgetOptions=generateMunicipalBudgetOptions;
    P.attachMunicipalBudgetOptions=function(decision,input){const budgets=generateMunicipalBudgetOptions(input);if(!decision||typeof decision!=='object')return{ok:false,code:'DECISION_REQUIRED'};decision.budgetOptions=budgets.options.slice();return{ok:true,scope:budgets.scope,options:budgets.options};};
    P.optimizeMunicipalPortfolio=function(decision,budgetId){const budgets=Array.isArray(decision&&decision.budgetOptions)?decision.budgetOptions:[], normalized=TYPE_KEYS[budgetId]||budgetId, chosen=budgets.find(x=>x.id===`municipal-budget:${normalized}`)||budgets.find(x=>x.type===budgetId)||budgets.find(x=>x.type==='FULL_MUNICIPAL');const envelope=chosen?chosen.amount:(decision&&decision.constraints&&decision.constraints.budget);return exactFixedCostPortfolio(decision,envelope);};
    return true;
  }
  function install(){if(w.VIDIK_PLATFORM_10)return attach(w.VIDIK_PLATFORM_10);return false;}
