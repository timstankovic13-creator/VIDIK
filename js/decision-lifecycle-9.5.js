...
(function(){
  ...
  async function persist(){
    ...
    let r=read();
    if(!r){r=makeRecord(d)} else {r.decisionObject=d;r.updatedAt=new Date().toISOString();r.version+=1;r.events.push({type:'RECOMPUTE',at:r.updatedAt,version:r.version})}
    r.integrityHash=await hash(r); write(r); render(r); return {ok:true,record:r}
  }
  async function snapshot(){
    ...
    snap.hash=await hash(snap); r.snapshot=snap;r.updatedAt=new Date().toISOString();r.events.push({type:'SNAPSHOT',at:r.updatedAt,hash:snap.hash});r.integrityHash=await hash(r);write(r);render(r);return {ok:true,snapshot:snap};
  }
  async function overrideDecision(){
    ...
    r.adoptedDecision={recommendation:value,source:'HUMAN_OVERRIDE'};r.status='ADOPTED';r.version+=1;r.updatedAt=new Date().toISOString();r.events.push({type:'HUMAN_OVERRIDE',at:r.updatedAt,version:r.version});r.integrityHash=await hash(r);write(r);await snapshot();return {ok:true,record:read()};
  }
  async function recordOutcome(){
    const r=read();if(!r)return {ok:false,reason:'Persist the decision first'};const predicted=Number(document.getElementById('lifecyclePredicted')?.value),observed=Number(document.getElementById('lifecycleObserved')?.value),checkpoint=document.getElementById('lifecycleCheckpoint')?.value;if(!Number.isFinite(predicted)||!Number.isFinite(observed))return {ok:false,reason:'Predicted and observed outcomes are required'};r.outcomes.push({checkpoint,predicted,observed,delta:observed-predicted,recordedAt:new Date().toISOString()});r.status='REVIEWED';r.version+=1;r.updatedAt=new Date().toISOString();r.events.push({type:'OUTCOME_REVIEW',checkpoint,at:r.updatedAt,version:r.version});r.integrityHash=await hash(r);write(r);render(r);return {ok:true,record:r};
  }
  async function verify(){const r=read();if(!r)return {ok:false,reason:'No persisted decision'};const copy=clone(r),expected=copy.integrityHash;delete copy.integrityHash;return {ok:expected===(await hash(copy)),expected,actual:await hash(copy)}}
