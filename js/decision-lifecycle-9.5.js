...
(function(){
  ...
  async function snapshot(){
    ...
    snap.hash=await hash(snap); r.snapshot=snap;r.updatedAt=new Date().toISOString();r.events.push({type:'SNAPSHOT',at:r.updatedAt,hash:snap.hash});r.integrityHash=await integrityHash(r);write(r);render(r);return {ok:true,snapshot:snap};
  }
  async function overrideDecision(){
    ...
    r.adoptedDecision={recommendation:value,source:'HUMAN_OVERRIDE'};r.status='ADOPTED';r.version+=1;r.updatedAt=new Date().toISOString();r.events.push({type:'HUMAN_OVERRIDE',at:r.updatedAt,version:r.version});r.integrityHash=await integrityHash(r);write(r);await snapshot();return {ok:true,record:read()};
  }
