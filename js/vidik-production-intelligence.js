...
function auditInterventionUniverse({ problem, candidates = [], sourceSearches = [], statusQuo = null } = {}) {
  ...
  const effectLeaks = normalized.filter(c => c.discovery?.effectsImported === true || c.discovery?.causalEffectImported === true || c.effect || c.causalEffect);
  ...
  return {
    schemaVersion: 'vidik.production-intervention-universe.v1',
    problem: String(problem || '').trim(),
    status,
    candidatesConsidered: normalized.length,
    uniqueCandidateNames: names.size,
    interventionFamilies: families.size,
    sourcesSearched: searched.length,
    sourceFailures: failures.length,
    leadOnlyCount: leadOnly.length,
    effectLeaks: effectLeaks.length,
    provenanceGaps: provenanceGaps.length,
    statusQuoPresent: statusQuo?.explicit === true,
    recommendationBoundary: effectLeaks.length === 0 && leadOnly.length === normalized.length ? 'discovery-only' : 'violation',
    universeHash: sha256({ problem, candidates: normalized.map(c => ({ id: c.id, name: c.name, discovery: c.discovery })) , sourceSearches })
  };
}
...
function auditEvidencePipeline({ candidates = [], evidenceLeads = [], verifications = {}, targetJurisdiction = null } = {}) {
  ...
  for (const lead of leads) {
    ...
    promotion[lead.id] = Promotion.assessEvidencePromotion({ lead, verification, targetJurisdiction });
  }
  ...
  return {
    schemaVersion: 'vidik.production-evidence-pipeline.v1',
    candidateCount: candidates.length,
    evidenceLeadCount: leads.length,
    verifiedParameterCount: eligible.length,
    orphanVerificationCount: orphanVerifications.length,
    unsafeLeadCount: unsafe.length,
    promotion,
    recommendationBoundary: unsafe.length === 0 ? 'lead-or-verified-parameter' : 'violation',
    verifiedParameterHash: sha256(eligible.map(g => g.verifiedParameter))
  };
}
...
function validateOptimization({ candidates = [], allocations = {}, budget = null, analysis = {} } = {}) {
  const rows = candidates.map(candidate => {
    ...
    return {
      id: candidate.id,
      allocation,
      finiteInputs: finite(effect) && positive(resource),
      uncertaintyValid: finite(low) && finite(high) && low <= high,
      marginalEfficiency: positive(resource) && finite(effect) ? effect / resource : null,
      budgetUsed: allocation
    };
  });
  const used = rows.reduce((sum, row) => sum + row.allocation, 0);
  const budgetValid = budget == null || (finite(budget) && used <= Number(budget) + 1e-9);
  const resourceValid = rows.every(row => row.allocation >= 0 && row.finiteInputs && row.uncertaintyValid);
  return {
    schemaVersion: 'vidik.production-optimization.v1',
    rows,
    budget: budget == null ? null : Number(budget),
    budgetUsed: used,
    budgetValid,
    resourceValid,
    validated: budgetValid && resourceValid,
    optimizationHash: sha256({ rows, budget })
  };
}
...
function buildSensitivityEnvelope({ candidates = [], analysis = {}, score = null } = {}) {
  ...
  return {
    schemaVersion: 'vidik.sensitivity-envelope.v1',
    rows,
    recommendation,
    recommendationFlip: new Set(Object.values(recommendation).filter(Boolean)).size > 1,
    stable: rows.length > 0 && rows.every(r => r.valid),
    sensitivityHash: sha256({ rows, recommendation })
  };
}
...
function calculateVOI({ sensitivity, decisionValue = null, informationCost = null } = {}) {
  ...
  const finiteInputs = finite(value) && value >= 0 && finite(cost) && cost >= 0;
  const expectedValue = finiteInputs && flip ? value : 0;
  return {
    schemaVersion: 'vidik.value-of-information.v1',
    recommendationFlip: flip,
    decisionValue: finiteInputs ? value : null,
    informationCost: finiteInputs ? cost : null,
    expectedValueOfInformation: finiteInputs ? expectedValue : null,
    netValueOfInformation: finiteInputs ? expectedValue - cost : null,
    actionable: finiteInputs && expectedValue > cost,
    unknownIsNotZero: !finiteInputs
  };
}
...
function buildLearningEnvelope({ artifactHash, observations = [] } = {}) {
  ...
  const residuals = valid.map(o => ({ metric: o.metric, residual: Number(o.observed) - Number(o.predicted), months: Number(o.months) || null }));
  return {
    schemaVersion: 'vidik.production-learning.v1',
    baselineArtifactHash: artifactHash || null,
    observations: residuals,
    observedCount: residuals.length,
    driftSignal: residuals.some(r => Math.abs(r.residual) > 0),
    attributionRequired: true,
    automaticParameterMutation: false,
    historyImmutable: true,
    learningHash: sha256({ artifactHash, residuals })
  };
}
...
function certifyDecision({ run, selectedCandidateId = null, verifications = {}, targetJurisdiction = null, budget = null, decisionValue = null, informationCost = null, observations = [] } = {}) {
  ...
  const optimization = validateOptimization({ candidates: run?.candidates || [], allocations, budget, analysis });
  ...
  const stages = {
    interventionUniverse: universe.recommendationBoundary !== 'violation' && universe.status !== 'not-searched',
    evidencePipeline: evidence.unsafeLeadCount === 0,
    quantitativeOptimization: optimization.validated,
    uncertaintySensitivityVOI: sensitivity.stable && Boolean(voi.schemaVersion),
  ...
  return {
    schemaVersion: 'vidik.production-intelligence-certification.v1',
    stages,
    complete: Object.values(stages).every(Boolean),
    blockers: Object.entries(stages).filter(([, ok]) => !ok).map(([id]) => id),
    universe,
    evidence,
    optimization,
    sensitivity,
    ...
    artifactValidation,
    learning,
    certificationHash: sha256({ stages, universe, evidence, optimization, sensitivity, voi, artifactValidation, learning })
  };
}
...
function runAdversarialTournament({ scenarios = [] } = {}) {
  ...
  for (const scenario of scenarios) {
    ...
    results.push({
      id: scenario.id || scenario.problem,
      candidatesSupplied: candidates.length,
      candidatesRetained: found.length,
      retainedIds: found.map(c => c.id),
      decoyLeakCount: retainedDecoys.length,
      unrelatedLeakCount: unrelatedLeak.length,
      emptyIsExplicit: found.length === 0,
      passed: retainedDecoys.length === 0 && unrelatedLeak.length === 0
    });
  }
...
module.exports = {
  auditInterventionUniverse,
  auditEvidencePipeline,
  validateOptimization,
  buildSensitivityEnvelope,
  calculateVOI,
  buildLearningEnvelope,
  certifyDecision,
  runAdversarialTournament,
  sha256
};
