...
const ClosedLoopProduction = require('./vidik-production-closed-loop');
...
function auditInterventionUniverse({ problem, candidates = [], sourceSearches = [], statusQuo = null } = {}) {
  ...
  const families = new Set(normalized.flatMap(c => [...(Array.isArray(c.interventionClasses) ? c.interventionClasses : []), ...(Array.isArray(c.domains) ? c.domains : []), ...(Array.isArray(c.problemTags) ? c.problemTags : [])].map(String).map(s => s.trim().toLowerCase()).filter(Boolean)));
...
function auditEvidencePipeline({ candidates = [], evidenceLeads = [], verifications = {}, targetJurisdiction = null } = {}) {
  const leads = evidenceLeads.filter(Boolean), leadIds = new Set(leads.map(l => l.id)), orphanVerifications = Object.keys(verifications || {}).filter(id => !leadIds.has(id)), promotion = {};
  ...
  const eligible = Object.values(promotion).filter(g => g.eligible), unsafe = leads.filter(l => l.evidenceLeadOnly !== true || l.causalEffectImported === true);
...
function validateOptimization({ candidates = [], allocations = {}, budget = null, analysis = {} } = {}) {
  const rows = candidates.map(candidate => { const a = analysis[candidate.id] || {}, allocation = Number(allocations[candidate.id] || 0), effect = Number(a.effect ?? a.estimate ?? a.parameter?.value), resource = Number(a.resource ?? a.marginal?.resource), low = Number(a.uncertainty?.low), high = Number(a.uncertainty?.high); return { id: candidate.id, allocation, finiteInputs: finite(effect) && positive(resource), uncertaintyValid: finite(low) && finite(high) && low <= high, marginalEfficiency: positive(resource) && finite(effect) ? effect / resource : null, budgetUsed: allocation }; });
  const used = rows.reduce((sum, row) => sum + row.allocation, 0), budgetValid = budget == null || (finite(budget) && Number(budget) >= 0 && used <= Number(budget) + 1e-9), resourceValid = rows.every(row => row.allocation >= 0 && row.finiteInputs && row.uncertaintyValid);
...
function buildSensitivityEnvelope({ candidates = [], analysis = {}, score = null } = {}) {
  ...
  const rows = candidates.map(candidate => { const a = analysis[candidate.id] || {}, base = Number(a.effect ?? a.estimate ?? a.parameter?.value), low = Number(a.uncertainty?.low), high = Number(a.uncertainty?.high), values = { low: scorer(candidate, low), base: scorer(candidate, base), high: scorer(candidate, high) }; return { id: candidate.id, values, valid: finite(base) && finite(low) && finite(high) && low <= high }; });
  const pick = key => rows.filter(r => r.valid).slice().sort((a, b) => Number(b.values[key]) - Number(a.values[key]))[0]?.id || null, recommendation = { low: pick('low'), base: pick('base'), high: pick('high') };
...
function calculateVOI({ sensitivity, decisionValue = null, informationCost = null } = {}) {
  const value = Number(decisionValue), cost = Number(informationCost), flip = Boolean(sensitivity?.recommendationFlip), finiteInputs = finite(decisionValue) && finite(informationCost) && value >= 0 && cost >= 0, expectedValue = finiteInputs && flip ? value : (finiteInputs ? 0 : null);
...
function buildLearningEnvelope({ artifactHash, observations = [] } = {}) {
  const valid = observations.filter(o => o && o.metric && finite(o.predicted) && finite(o.observed)), residuals = valid.map(o => ({ metric: o.metric, residual: Number(o.observed) - Number(o.predicted), months: finite(o.months) ? Number(o.months) : null }));
...
function validateCanonicalDecisionIntegration({ resourceEnvelope, interventionComparison = [], resourceModels = {}, baseline = {}, parameters = [], correlations = [], scoreFn, voiCandidates = [], decisionValue = 1, sensitivitySteps = 21, uncertaintySamples = 500, expectedOptimizerStatus = 'OPTIMIZED' } = {}) {
  const optimization = ResourceOptimization.evaluateResourceOptimization(resourceEnvelope, interventionComparison, resourceModels); let analysis = null, analysisError = null;
  try { analysis = DecisionIntelligence.analyze({ baseline, parameters, correlations, scoreFn, candidates: voiCandidates, decisionValue, sensitivitySteps, uncertaintySamples }); } catch (error) { analysisError = error.message; }
  const analysisValid = Boolean(analysis && analysis.version === DecisionIntelligence.VERSION && analysis.integrityHash && analysis.integrityHash.length === 64), optimizerValid = optimization.status === expectedOptimizerStatus && Array.isArray(optimization.candidates);
  return { schemaVersion: 'vidik.canonical-decision-integration.v1', optimizer: optimization, decisionIntelligence: analysis, analysisError, optimizerValid, analysisValid, validated: optimizerValid && analysisValid, integrationHash: sha256({ optimization, analysis, analysisError }) };
}
...
function certifyDecision({ run, selectedCandidateId = null, verifications = {}, targetJurisdiction = null, budget = null, decisionValue = null, informationCost = null, observations = [], tournament = null, canonicalIntegration = null } = {}) {
  ...
  const evidenceLeads = Object.values(run?.evidenceDiscovery || {}).flatMap(item => item?.evidenceLeads || []).filter(Boolean), evidence = auditEvidencePipeline({ candidates: run?.candidates || [], evidenceLeads, verifications, targetJurisdiction });
  const analysis = run?.analysis || Object.fromEntries((run?.candidates || []).map(c => [c.id, run?.analysisInputs?.[c.id] || {}])), optimization = validateOptimization({ candidates: run?.candidates || [], allocations: run?.allocations || {}, budget, analysis }), sensitivity = buildSensitivityEnvelope({ candidates: run?.candidates || [], analysis }), voi = calculateVOI({ sensitivity, decisionValue, informationCost }), selectedArtifact = selectedCandidateId ? run?.governance?.decisionArtifacts?.[selectedCandidateId] : null, artifactValidation = selectedArtifact ? Closure.validateDecisionArtifact(selectedArtifact) : { valid: false, reasons: ['selected-artifact-missing'] }, learning = buildLearningEnvelope({ artifactHash: selectedArtifact?.artifactHash || null, observations });
  const stages = { interventionUniverse: universe.candidatesConsidered > 0 && universe.recommendationBoundary !== 'violation' && universe.status === 'found' && universe.sourceFailures === 0, evidencePipeline: evidence.evidenceLeadCount > 0 && evidence.verifiedParameterCount > 0 && evidence.unsafeLeadCount === 0 && evidence.orphanVerificationCount === 0, quantitativeOptimization: optimization.validated, uncertaintySensitivityVOI: sensitivity.stable && voi.inputsComplete, decisionArtifact: artifactValidation.valid, outcomeLearning: learning.historyImmutable && learning.automaticParameterMutation === false, adversarialReadiness: Boolean(tournament?.passed === true) };
...
function runAdversarialTournament({ scenarios = [] } = {}) {
  const results = [];
  for (const scenario of scenarios) { const candidates = Array.isArray(scenario.candidates) ? scenario.candidates : [], found = Discovery.discoverInterventions({ problem: scenario.problem, candidates }), decoys = candidates.filter(c => c.decoy === true).map(c => c.id), retainedDecoys = found.filter(c => decoys.includes(c.id)).map(c => c.id), unrelatedLeak = found.filter(c => c.problemTags?.includes('unrelated')).map(c => c.id); results.push({ id: scenario.id || scenario.problem, candidatesSupplied: candidates.length, candidatesRetained: found.length, retainedIds: found.map(c => c.id), decoyLeakCount: retainedDecoys.length, unrelatedLeakCount: unrelatedLeak.length, emptyIsExplicit: found.length === 0, passed: retainedDecoys.length === 0 && unrelatedLeak.length === 0 }); }
...
module.exports = { auditInterventionUniverse, auditEvidencePipeline, validateOptimization, buildSensitivityEnvelope, calculateVOI, buildLearningEnvelope, validateCanonicalDecisionIntegration, certifyDecision, runAdversarialTournament, sha256, productionClosedLoop: ClosedLoopProduction.executeProductionDecision, buildProductionDecisionAnalysis: ClosedLoopProduction.buildCanonicalAnalysis, blindProductionAssertion: ClosedLoopProduction.assertBlindRun };