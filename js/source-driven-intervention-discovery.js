...
function buildGovUkSearchUrl(source, query, { rows = 10 } = {}) { if (!source?.url || !GOVUK_SOURCE_IDS.has(source.sourceId)) throw new Error('unsupported-govuk-intervention-source'); if (!String(query || '').trim()) throw new Error('source-driven-query-required'); if (!Number.isInteger(rows) || rows < 1 || rows > 100) throw new Error('source-driven-page-size-invalid'); const url = new URL(source.url); url.searchParams.set('q', String(query).trim()); url.searchParams.set('count', String(rows)); url.searchParams.set('fields', 'title'); url.searchParams.set('fields', 'description'); url.searchParams.set('fields', 'link'); url.searchParams.set('fields', 'format'); return url.toString(); }
...
function isActionableInterventionTitle(title,notes=''){
  const titleText=normalizeText(title).toLowerCase(), text=normalizeText(title+' '+notes).toLowerCase();
  ...
  const explicitProgram=/\b(program|programme|service|initiative|intervention|pilot|project|grant|funding|subsidy|benefit|voucher|training|clinic|shelter|treatment|outreach|enforcement|patrol|assistance|support|response|reform|modernization|automation|navigation|assessment|governance)\b/i.test(titleText);
  const concreteAction=/\b(provide|expand|deploy|implement|operate|fund|subsidize|regulate|inspect|train|hire|staff|build|install|retrofit|convert|redesign|reduce|increase|improve|prevent|manage|maintain)\b/i.test(titleText);
...
function extractOpenAlexInterventionLeads(payload, source, problem, workspace = 'municipal', query = '') {
  ...
  const terms = [...new Set([...taxonomy, ...explicit])];
  const leads = [];
  for (const row of rows.slice(0, 20)) {
    ...
    for (const term of [...new Set([...matched, ...fallbackTerms])].slice(0, 2)) {
      ...
      leads.push({
        id: `source:${source.sourceId}:${row.id || row.doi || canonicalName}`,
        name,
        canonicalName,
        interventionFamily: inferInterventionFamily(name),
        ...
        discovery: {
          ...
          sourceType: 'intervention-literature',
...
async function discoverSourceDrivenInterventions({problem,jurisdiction=null,workspace='municipal',sources=null,fetchImpl,now=new Date(),rows=25}={}){
  ...
  if (allowLiteratureFallback && (candidates.length === 0 || (coverage.expectedFamilies.length && coverage.coverageRatio < 0.5))) {
    ...
    if (literatureSource) {
      const literatureQueries = [...new Set([problem, ...taxonomyTerms(problem, workspace).slice(0, 4).map(term => `${problem} ${term}`)])].slice(0, 3);
      const attempts = [];
