...
function isActionableInterventionTitle(title,notes='',{allowDescriptionSignals=false}={}){
  ...
  const serviceOnly=/\bservice\b/i.test(titleText) && !/\b(program|programme|initiative|intervention|pilot|scheme|voucher|subsidy|training|clinic|shelter|treatment|outreach|enforcement|patrol|assistance|support|response|reform|modernization|automation|navigation)\b/i.test(titleText);
