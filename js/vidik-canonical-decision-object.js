...
function buildCanonicalDecisionObject(run) {
  ...
  const object = {
    ...
    causalProductionModel: { chain: ['marginal_resource', 'capacity', 'activity', 'immediate_outcome', 'system_outcome', 'serious_harm_pathway'], selectedParameter: parameter, observedMunicipalDataIsNotCausal: true },
    ...
    optimizationOpportunityCost: { marginalResourceOptimization: false, status: 'requires-resource-quantity-and-comparable-marginal-effects', competingInterventions: interventions.map(x => ({ id: x.id, status: x.status, score: x.score })) },
    rationale: { recommendation: run?.recommendation || null, why: selected ? `${selected.name} is admissible and highest-scoring among the evaluated admissible interventions.` : 'No intervention cleared the admissibility gates.', whyNot: run?.audit?.blockedAlternatives || [] },
    ...
    reoptimizationExecutionReadiness: { reoptimization: 'available-after-observed-outcome-or-new-evidence', executionReadiness: 'separate-assessment-required', evidenceAcquisition: city ? { targetCity: city, status: run?.recommendation ? 'not-required-for-selected-claim' : 'required' } : null }
  };
