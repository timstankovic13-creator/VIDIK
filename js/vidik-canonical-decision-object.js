...
function buildDecisionIntelligence(run, interventions) {
  ...
  const scoreFn = values => { const rows = candidates.map(x => { const current = currentEffects[x.id], effect = Number(values[`${x.id}:effect`] ?? current), risk = Number(x.risk || 0), score = baseScores[x.id] + (effect - current) * (1 - risk); return { id: x.id, score, name: x.name }; }).filter(x => Number.isFinite(x.score)).sort((a, b) => b.score - a.score || a.id.localeCompare(b.id)); return rows.length ? { recommendation: rows[0].id, score: rows[0].score, ranking: rows } : { recommendation: null, score: null, ranking: [] }; };
  const voiCandidates = parameters.map(p => { const lowResult = scoreFn({ ...baseline, [p.id]: p.low }), highResult = scoreFn({ ...baseline, [p.id]: p.high }); return { id: p.id, currentValue: scoreFn(baseline).score, lowValue: lowResult.score, highValue: highResult.score, pHigh: .5, cost: 0, lowRecommendation: lowResult.recommendation, highRecommendation: highResult.recommendation, recommendations: [lowResult.recommendation, highResult.recommendation] }; });
